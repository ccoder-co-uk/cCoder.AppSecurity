...
public partial class PrivilegeProcessingServiceTests
{
    ...
    public async Task ShouldThrowInvalidOperationExceptionWhenUserHasCreatePrivilegeForAddAsync()
    {
        ...
        User user = WithPrivilege("privilege_create");

        currentUser = user;

        // When
        Func<Task> act = async () => await privilegeProcessingService.AddPrivilegeAsync(privilege);

        // Then
        await act.Should()
            .ThrowAsync<InvalidOperationException>()
            .WithMessage("Cannot add privileges");
