...
internal sealed class AcceptanceDatabaseManager(IServiceProvider services)
{
    ...
    private static void EnsureSafeAcceptanceDatabase(string connectionString, string protectedDatabaseName)
    {
        ...
        {
            throw new InvalidOperationException(message: "Acceptance database connection string is empty.");
        }
        ...
        {
            throw new InvalidOperationException(message: "Acceptance database name is empty.");
        }
...
message: $"Refusing to run acceptance database operations against protected database '{protectedDatabaseName}'."
            );
        }
        ...
        if (!databaseName.Contains(value: "accept", comparisonType: StringComparison.OrdinalIgnoreCase))
        {
            ...
            throw new InvalidOperationException(
message: $"Refusing to run acceptance database operations against non-acceptance database '{databaseName}'."
            );
        }
    }
